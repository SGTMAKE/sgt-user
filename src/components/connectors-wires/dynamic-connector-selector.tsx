...
interface DynamicConnectorSelectorProps {
  type: "connectors" | "wires"
  onAddToQuote: (data: any) => void
  isLoading?: boolean
}

export default function DynamicConnectorSelector({
  type,
  onAddToQuote,
  isLoading = false,
}: DynamicConnectorSelectorProps) {
  const [categories, setCategories] = useState<ConnectorCategory[]>([])
  ...
  const fetchCategories = async () => {
    try {
      const response = await fetch(`/api/connectors/categories?type=${type}`)
  ...
  const handleFormSubmit = (data: any) => {
    ...
    const quoteItem = {
      type: type === "connectors" ? "connector" : "wire",
    ...
    setQuantity(1)
    toast.success(`${type === "connectors" ? "Connector" : "Wire"} added to quote request`)
  }

  const generateTitle = (category: ConnectorCategory, data: any) => {
  ...
  const renderOptionInput = (option: ConnectorOption) => {
  ...
  return (
    <div className="space-y-6">
      {/* Category Selection */}
      <div>
        <h2 className="text-xl font-medium mb-4">Select {type === "connectors" ? "Connector" : "Wire"} Category</h2>
