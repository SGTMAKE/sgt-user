...
const OrderConfirmed = async ({ params }: { params: { oid: string } }) => {
  ...
  return (
    <div className="mx-auto mb-10 flex max-w-2xl flex-col items-center rounded-xl bg-white p-5 md:p-10">
      ...
      <h1 className="mb-1 mt-5 text-xl font-semibold">We received your order!</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2">
        <div className="order-1 mt-5 md:mt-0">
          <h1 className="text-xs font-semibold uppercase tracking-widest text-gray-400">Payment</h1>
          <p className="mt-3 text-sm">{order.order.method}</p>
          <p className="text-sm">{order.order.via}</p>
        </div>
        <div className="-order-1 md:order-2">
          <h1 className="text-xs font-semibold uppercase tracking-widest text-gray-400">Shipping address</h1>
      ...
      <div className="w-full">
        <h1 className="text-xs font-semibold uppercase tracking-widest text-gray-400">Order items</h1>
        <div className="py-4">
...
function OrderItem(orderItem: ItemSummary) {
  ...
  const isCustomProduct = !!orderItem.isCustomProduct

  // Determine the image source based on whether it's a custom product
  const imageSrc = isCustomProduct
    ? orderItem.imageUrl // For custom products, imageUrl is already the full URL
    : process.env.NEXT_PUBLIC_IMAGE_URL + orderItem.imageUrl // For regular products, prepend the base URL
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="grid flex-1 grid-cols-1 gap-3 md:grid-cols-2">
          <div>
            ...
            {isCustomProduct && orderItem.customProductData?.options && (
              <div className="mt-1">
                {Object.entries(orderItem.customProductData.options).map(([key, value]) => {
